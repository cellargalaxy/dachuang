...
//			String name = tarArchiveEntry.getEvidenceName();
...
//		String basePath = file.getEvidenceName();
//		if (file.isDirectory()) {
...
//				if (file.isDirectory()) {
//					archiveFolder(file, tarArchiveOutputStream, basePath + File.separator + file.getEvidenceName());
//				} else {
//					archiveFile(tarArchiveOutputStream, file, basePath);
//				}
//			}
...
//			archiveFile(tarArchiveOutputStream, inputStream, file.length(), basePath + File.separator + file.getEvidenceName());
//		}
//	}
//
//	public static final void archiveFile(TarArchiveOutputStream tarArchiveOutputStream, File file) throws IOException {
//		try (InputStream inputStream = IOUtils.getInputStream(file)) {
//			archiveFile(tarArchiveOutputStream, inputStream, file.length(), file.getEvidenceName());
//		}
//	}
//
...
//	}
	...
	public static final BufferedWriter getWriter(File file) throws IOException {
		return getWriter(file, DEFAULT_CODING);
	}

	public static final BufferedReader getReader(File file) throws FileNotFoundException, UnsupportedEncodingException {
		return getReader(file, DEFAULT_CODING);
	}

	public static final BufferedWriter getWriter(File file, String coding) throws IOException {
		if (file.getParentFile() != null && !file.getParentFile().exists()) {
			file.getParentFile().mkdirs();
		}
		return new BufferedWriter(new OutputStreamWriter(getOutputStream(file), coding));
	}

	public static final BufferedReader getReader(File file, String coding) throws FileNotFoundException, UnsupportedEncodingException {
		return new BufferedReader(new InputStreamReader(getInputStream(file), coding));
	}
