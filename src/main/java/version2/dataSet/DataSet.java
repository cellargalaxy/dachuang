...
//	}
	...
	public static final void outputDataSet(DataSet dataSet,DataSet subDataSet, File outputDataSet,RunParameter runParameter,
	                                       double subSpaceAuc,DsCount dsCount,AucCount aucCount,
		...
		for (Map.Entry<DataSet, double[]> entry : subSpaceMap.entrySet()) {
			csvPrinter.printRecord(entry.getKey().getEvidenceNums(),aucCount.countOrderAuc(entry.getKey(),entry.getValue()),Arrays.toString(entry.getValue()));
		}
